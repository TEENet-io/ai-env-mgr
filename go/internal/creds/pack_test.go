...
func TestPackIsDeterministic(t *testing.T) {
	set := model.CredentialSet{
		...
		model.PathCodexConfig: []byte("config"),
	}
	first, err := Pack(set)
	if err != nil {
		t.Fatalf("Pack: %v", err)
...
func TestMergeOverlaysWithoutLosingEntries(t *testing.T) {
	existing := model.CredentialSet{
		...
		model.PathCodexModels: []byte("old models"),
	}
	incoming := model.CredentialSet{
		model.PathCodexModels: []byte("new models"),
	}
	...
	if string(merged[model.PathCodexModels]) != "new models" {
...
func TestMergeDoesNotMutateInputs(t *testing.T) {
	...
	incoming := model.CredentialSet{model.PathCodexModels: []byte("models")}
