...
func (s *Server) handleAdmins(w http.ResponseWriter, r *http.Request, sess *session) {
	...
	// A password generated by the previous request is carried through the
	// redirect in the query string: once, and gone with the next reload.
	data.NewPassword = r.URL.Query().Get("pw")
	data.NewUsername = r.URL.Query().Get("u")
	s.render(w, "admins.html", http.StatusOK, data)
}

// handleAdminCreate is a POST like the others, except that what it carries
// through the redirect is a generated password rather than a notice.
func (s *Server) handleAdminCreate(w http.ResponseWriter, r *http.Request, sess *session) {
	...
	query, err := s.actionAdminCreate(sess, r)
	if err != nil {
		s.redirectWithError(w, r, "/admins", err.Error())
		return
	}
	http.Redirect(w, r, "/admins?"+query, http.StatusSeeOther)
}

func (s *Server) actionAdminCreate(sess *session, r *http.Request) (string, error) {
	if sess.admin.Role != repo.RoleAdmin {
		return "", errors.New("只有 admin 角色可以添加管理员")
	}
	username := formValue(r, "username")
	if username == "" {
	...
	// Returned as the redirect's query; the page shows it once.
	return "pw=" + url.QueryEscape(password) + "&u=" + url.QueryEscape(admin.Username), nil
}
