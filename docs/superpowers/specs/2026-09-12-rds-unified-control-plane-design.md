...
> 日期：2026-09-06 · rev.4 修订 2026-09-18（控制面落点已定）；rev.3 修订 2026-09-12（含 5 处修正与 3 项决定）  
...
**rev.4（2026-09-18）：控制面回阿里云。** 原控制台主机（阿里云 ECS 47.236.115.50）于 2026-09-17 销毁，控制台临时部署在 AWS 网关机 175.41.186.104 上（与 LiteLLM、Dify 共用，可用内存不足 1 GB，探测不再独立）。rev.3 §13 定的"迁到 SLS/OSS 所在阿里云账号后用实例角色"因此改为明确决定：

1. **控制面（控制台 API + Worker + RDS）落在阿里云账号 5230362917187930（OSS/SLS 所在账号），地域新加坡 ap-southeast-1**，新开一台 ECS 承载，数据库用同私网的阿里云 RDS PostgreSQL。
2. **OSS、SLS、KMS 一律用该 ECS 的 RAM 实例角色**，不再有静态 AccessKey；登录 AK 与 `AliyunLogReadOnlyAccess` 在切换后收回。
3. **模型网关继续留在 AWS**（Bedrock 用 EC2 实例角色，与 Vertex、Azure 的密钥都在网关机上），控制面经公网调它的管理接口，凭 proxy_admin 密钥（服务端环境变量）。
4. 现部署在 AWS 网关机上的控制台是过渡状态，新 ECS 就绪后迁走；迁移后网关机只跑网关，探测恢复独立（放控制面 ECS）。

跨云边界因此是明确的两条：控制面→网关（HTTPS + 管理密钥）、员工端→网关（HTTPS + 员工令牌）。业务数据、文件、日志、密钥全部在阿里云一侧。
...
- ~~服务端访问云资源的身份形式~~ **[rev.4 已定 2026-09-18]**：控制面落在阿里云账号 5230362917187930、地域 ap-southeast-1 的新 ECS 上，OSS、SLS、KMS 用该实例的 RAM 角色；LiteLLM 管理密钥继续走服务环境变量（网关在 AWS，只能用密钥）。过渡期（控制台暂在 AWS 网关机）用受限 AK。
