...
此外再加系统策略 **`AliyunECDReadOnlyAccess`**：控制台用同一把密钥查询无影云电脑状态，把"休眠"和"agent 挂了"分开。**这条只给管理员用户** —— agent 的密钥在每台员工机器上是明文的，给它 ECD 权限等于让任意一台机器枚举全公司的云电脑。
...
| `codexRolloutPct` | **已废弃,恒为 100**。曾是按 `crc32(主机名) % 100` 判定的灰度比例。仍然写出,因为 agent 1.2.5–1.2.7 会拿它当开关,字段缺失或为 0 时**一台都不装且不报错**;新 agent 忽略它 |
...
**和 §7 一样是全量下发**。曾经这里有灰度,理由是这个包对上游 Codex 的重打包会随版本漂移,CI 通过只能证明补丁正确应用,**不能证明 ChatGPT 入口真的消失或 Computer Use 仍可用**。这条理由仍然成立,但灰度换不来它:机器数量少,10% 常常一台都不命中,而真正的验收只能在 Windows 真机上做。**先真机验收,再发布。**

**权限**:agent 对 `_codex/*` 只需 **GetObject**(读)。**只给读,绝不给写**——理由同 §7。

**已实现**:agent 自 **1.2.5** 起会下载、校验 SHA-256、静默安装并上报实际装到的版本。设计见 [Codex分发方案](Codex分发方案.md)。包版本已与上游版本区分开(`<应用版本>-b<构建号>`);**安装器仍是每用户级**(`PrivilegesRequired=lowest`),目前能用是因为镜像把 Codex 装在 `C:\tools\Codex`。
