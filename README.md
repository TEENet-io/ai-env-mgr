...
- 控制台可下发 AppLocker 放行目录，agent 每个同步周期校对一次
...
| `dist/admin.exe` | 管理员工作机或一台内网服务器 | 管理控制台(浏览器):总览、员工账号、下发策略、发布更新 |
...
| 页面 | 能做什么 |
|---|---|
| 总览 | 机队状态、当前策略、模型网关,外加绑定/解绑/注销与某台机器的日志 |
| 员工账号 | 登记、停用(**并吊销凭据**)、启用 |
| 封禁策略 | 增删封禁域名、临时全局开关 |
| 会话采集 | 同步间隔、采集开关与统计 |
| 发布更新 | 发布 agent / Codex 到机队(需打字确认,Codex 有灰度) |
| 日志 | 网关调用明细与花费(需配 SLS 日志项目) |
