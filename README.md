...
**受控 AI 沙盒** —— 让员工在企业云电脑上使用定制版 Codex，经公司模型网关调用模型，同时做到：

- **员工无需厂商登录** —— 控制台签发员工自己的网关令牌，配置与模型目录由 agent 自动投递
- **浏览器进不去外部 AI 网站** —— 企业策略 + AppLocker，标准用户无法解除
- **离职能远程吊销** —— 控制台撤回网关令牌、删除下发凭据并解绑；agent 后续同步清理本地配置。云桌面访问回收与 Windows 注销另行执行
...
| 封禁策略 | 增删封禁域名、临时全局开关 |
| 会话采集 | 同步间隔、采集开关与统计 |
| 发布更新 | 发布 agent / Codex 到机队（需打字确认；新版先单机验收，再全量发布） |
...
- **网关与桌面分工** —— 上游厂商凭据留在网关，员工只接收自己的网关令牌；历史厂商登录凭据相关兼容代码不作为当前开户流程。
- **休眠唤醒立即同步** —— 定时器在休眠时冻结，所以叠加了电源事件和墙钟超期检查。

## 配置变更与失败恢复

- 员工原获准模型全部下架时，补发令牌会拒绝并要求管理员明确选择模型；不会自动扩展到所有可见模型。
- 发布配置先读取并合并原凭据包。只有明确不存在才新建；超时、权限错误或损坏 ZIP 都停止写入，保留原包并报告错误。
- 模型权限依次写入网关用户、令牌及下发目录。这不是跨系统原子事务：失败提示指出已执行阶段，审计保留失败尝试；管理员用相同模型选择重试完成同步，不自动恢复已收紧权限。
- 仅目录变化时，agent 写入新目录但不结束当前任务；员工下次启动 Codex 加载新目录。网关已移除的模型会立即拒绝新请求，目录显示暂旧不代表仍有权限。
- 令牌或其他受管配置变化仍会结束相关工具进程，管理员应提前通知员工保存工作。吊销由网关立即执行，不等待客户端重启。本文规则须在部署相应新版 agent 后生效；Windows 实机验收仍需完成。
