...
> ⚠️ **破坏性变更:命令行和 TUI 已移除。** 所有操作改在浏览器控制台里完成。
> 不带参数运行 `admin.exe` 即在本机开控制台(`http://127.0.0.1:8080`),
> 和以前裸跑进菜单是同一个动作,不依赖任何部署好的服务器。

**为什么删**

同一批操作维护两个前端,两边一直在各自漂移,而**每一次分歧都是有人用的时候撞上的,没有一次是看代码发现的**:

- 机器状态在控制台里被压成 6 种(命令行有 9 种),"静默 10 分钟"还被标成了"失联"
- Claude 代员工登录用了 Codex 的回调解析器——**在控制台里从来就不可用**
- release 下载有两份实现,错误信息已经不一致
- 状态与失败的区分只在一边生效

控制台已覆盖原菜单全部 8 项,共用逻辑(`admincore`)一行未动,删除的只是命令行专属代码:净减 2439 行。

**云电脑状态查询(新)**

控制台会向阿里云无影查询云电脑状态,回答一个 agent 答不了的问题:**一台安静的机器,是睡着了还是挂了**。

无影在**虚拟化层**挂起整台机器,客户机 Windows 收不到任何电源事件——实测确认:休眠再唤醒,本地日志里既无生命周期记录、也无唤醒同步,只有两次定时同步之间的一段空白。**agent 从头到尾没被通知过**,它没有机会留下"遗言"。
...
第三行是关键:机器开着、平台够得着、agent 却不吭声——**在能问到平台之前,它和"下班关机"长得完全一样**。
...
发布要把几百 MB 下载再上传,原来是在 HTTP 请求里同步做完的——Codex 安装包必然失败:服务端 2 分钟就截断响应,前面的代理更早放弃,浏览器卡死后工作还在后台跑,**没人知道机队到底有没有被推新二进制**。

现在提交即返回,页面每 5 秒自刷显示进度(下载 → 上传 → 完成),可以直接关掉页面。同时只允许一个发布在跑——两个并发会抢着写同一份策略,后完成的那个会悄悄覆盖先前的。

**文档**

`操作手册.md` §3.2 重写为控制台说明,领域知识全部保留(为什么停用是真吊销、为什么 `claude.json` 不能少、为什么临时开放是删注册表键)。§5.4 按实测重写——原文里"真休眠是尽力而为"这句话在无影上是错的。
