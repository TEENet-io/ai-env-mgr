...
Status 原本只有一个 `errors` 列表，于是"还没有人替这位员工登录"和"策略应用失败"并排放在一起，
日志对一台毫无问题的机器写 `errors=1`——其中一条甚至是成功通知（离职后已撤销凭证）。

- 五类**状态**移到新的 `warnings`：机器未分配、员工无用户目录、因此跳过凭证投放、凭证已撤销、还没人代登录。
- `errors` 从此只表示**失败**：策略拉取/解析/应用失败、凭证下载/解包/投放失败、自更新校验不符等。红色状态因此重新变得值得一看。
- 同步日志分开计数（`errors=0 warnings=1`），错误用 `!` 前缀、提示用 `-`。
- 新增状态 **`NO LOGIN YET` / 待代登录**（黄色）：机器已绑定、员工有用户目录，但还没人替他登录。
- 控制台**同时认识新旧两种格式**，机队还没升级到 1.2.6 时不会一片红。
- 日志页顶部分开列出该机器的错误与提示，不必在几百行 `sync ok` 里翻找。

**发布 Codex 时的 404**

粘贴 releases 页面上的下载地址、配 PAT 会得到 404：GitHub 的 release 资产由另一台主机分发，
那台主机不认 `Authorization` 头。现在会**自动转换成 API 资产地址**，照常粘贴即可。
失败时也会说清是哪一种：release 不存在、令牌读不到该仓库（并指出细粒度令牌需要 `Contents: Read`）、
还是文件名打错（并列出该 release 实际有哪些资产）。

下载实现原本 CLI 与 Web 各一份、错误信息已经不一致，现已合并为 `internal/ghrelease` 一份。

**agent**

- Codex 磁盘空间不足改为推迟（`deferred`），不再记成错误——机器没坏，只是今天没地方装。
